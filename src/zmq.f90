MODULE ZMQ
  ...
  INTERFACE
    ...
    FUNCTION ZMQ_Z85_DECODE(DEST, STRING) BIND(C, NAME = 'zmq_z85_decode')
    ...
    FUNCTION ZMQ_Z85_ENCODE(DEST, DATA, SIZE) BIND(C, NAME = 'zmq_z85_encode')
    ...
    FUNCTION FZMQ_POLL(ITEMS, NITEMS, TIMEOUT) BIND(C, NAME = 'zmq_poll')
      ...
      INTEGER(KIND = C_INT) :: FZMQ_POLL
    END FUNCTION FZMQ_POLL
...
CONTAINS
  ...
  FUNCTION ZMQ_POLL(ITEMS, NITEMS, TIMEOUT) RESULT(SIGNALED)
    TYPE(ZMQ_POLLITEM_T), DIMENSION(*), INTENT(INOUT) :: ITEMS
    ...
    CODE = FZMQ_POLL(ITEMS, NITEMS, TIMEOUT)
    SIGNALED = CODE > 0
